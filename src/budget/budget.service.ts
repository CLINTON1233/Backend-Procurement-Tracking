...
async updateBudget(id: number, data: any) {
  try {
    ...
    if (data.total_amount && Number(data.total_amount) !== Number(existingBudget.total_amount)) {
      ...
      const totalAmountIdr = convertToIDR(Number(data.total_amount), currency);
      ...
      updateData.remaining_amount_idr = totalAmountIdr - Number(existingBudget.used_amount_idr);
    }

    if (data.reserved_amount !== undefined) {
      updateData.reserved_amount = data.reserved_amount;
      const currency = data.currency || existingBudget.currency;
      updateData.reserved_amount_idr = convertToIDR(Number(data.reserved_amount), currency);
    }
